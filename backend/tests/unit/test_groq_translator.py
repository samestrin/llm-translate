"""
Unit tests for the Groq translator.
"""
import pytest
...
@pytest.fixture
def translator(mock_groq_client):
    """Create a GroqTranslator instance with a mock client."""
    translator = GroqTranslator(api_key="test-api-key", model="meta-llama/llama-4-maverick-17b-128e-instruct")
    translator.client = mock_groq_client
    return translator


@pytest.mark.asyncio
async def test_translate_success(translator, mock_groq_client):
    ...
    result = await translator.translate("Hello", "English", "Spanish")
    ...
    assert call_args["model"] == "meta-llama/llama-4-maverick-17b-128e-instruct"
    assert len(call_args["messages"]) == 2
    assert call_args["messages"][0]["role"] == "system"
    assert call_args["messages"][1]["role"] == "user"
    assert "Translate the following text from English to Spanish" in call_args["messages"][1]["content"]


@pytest.mark.asyncio
async def test_translate_authentication_error(translator, mock_groq_client):
    """Test handling of authentication errors."""
    # Create a mock response and request for the error
    mock_response = MagicMock()
    ...
    with pytest.raises(TranslationError) as excinfo:
        await translator.translate("Hello", "English", "Spanish")
...
@pytest.mark.asyncio
async def test_translate_rate_limit_error(translator, mock_groq_client):
...
@pytest.mark.asyncio
async def test_translate_bad_request_error(translator, mock_groq_client):
...
@pytest.mark.asyncio
async def test_translate_connection_error(translator, mock_groq_client):
    ...
    # Set up the mock to raise the error
    mock_groq_client.chat.completions.create.side_effect = connection_error
